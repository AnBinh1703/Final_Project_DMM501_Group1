...
✓ **POST /predict**: Accepts PredictRequest (feature list), returns PredictResponse with risk_score, risk_tier/action, threshold_review/high, model_version, request_id
...
- **Live Feed**: Last 50 predictions shown in reverse chronological order
  - Time, Amount, Risk Score, Tier (LOW/REVIEW/HIGH), Action (allow/review/block)
  - Color: Red bg for High, Orange bg for Review, neutral for Low
  ...
  while (streamState.isRunning) {
    ...
    try {
      ...
      const feedItem = {
        ...
        probability: (result.risk_score * 100).toFixed(1),
...
5. ✅ POST /predict with valid transaction returned risk_score, risk_tier/action, thresholds (review/high), model_version, request_id
...
4. ✅ Result displayed risk_score, risk_tier/action, thresholds (review/high), model_version, request_id
...
def predict(req: PredictRequest):
  ...
  6. Return: PredictResponse(risk_score=score, risk_tier=..., action=..., ...)
```
