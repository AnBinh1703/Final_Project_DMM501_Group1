...
\date{Generated from repository implementation}
...
\midrule
Logistic Regression & 0.99 & 0.6742 & 0.8108 & 0.7362 & 0.9680 & 0.7929 & Selected in deployed artifacts \\
LightGBM & 0.84 & 0.8852 & 0.7297 & 0.8000 & 0.8828 & 0.7161 & Benchmarked candidate \\
\bottomrule
\end{tabular}
\caption{Model comparison from repository benchmark artifacts.}
\end{table}
...
For storage, the system supports two repository modes. The in-memory repository is fully implemented and used as the default demo mode. The SQL repository is implemented in code and covered by integration testing. Docker Compose is prepared for PostgreSQL. It is therefore most accurate to describe SQL-backed persistence as partially implemented for production use.
...
SQL/Postgres repository & Durable case storage path & \code{src/repositories/sql_case_repository.py} & Partially implemented \\
...
The repository currently contains 33 test functions under \code{tests/}. In the present workspace, integration-style checks pass for multiple API and workflow scenarios, but a full plain \code{pytest} run encountered Windows temp-directory permission issues affecting tests that use \code{tmp_path}. This is an environment constraint rather than evidence that the implemented API workflow is absent.
...
SQL/Postgres persistence & Partially implemented & Repository and integration test exist; not the default demo path \\
