...
\begin{itemize}
  ...
  \item Data sources:
    \begin{itemize}
      \item Real sample stream: \code{frontend/real-samples.json} (pre-extracted rows from \code{data/archive/creditcard.csv}).
...
\begin{itemize}
  \item Risk levels: \emph{Normal} if score \(< 0.05\); \emph{Fraud} if score \(\ge\) backend threshold; otherwise \emph{Suspicious}.
  \item Alerts are created for \emph{Suspicious} and \emph{Fraud} rows and shown in the Alert Panel.
\end{itemize}
...
\begin{itemize}
  ...
  \item Average Fraud Score
  \item Fraud Predictions by Label
\end{itemize}
...
\begin{itemize}
  \item False positives: customer friction; mitigated via explicit threshold policy and a \emph{Suspicious} tier in the UI (human-in-the-loop workflow).
...
\begin{itemize}
  \item Header: connection pill + model version + threshold + expected feature count.
  \item Control Panel: API Base URL, mode (real/random), speed, start/stop/reset.
  \item Live Summary KPIs: total processed, fraud detected, suspicious, fraud rate, avg probability, last latency.
  \item Alert Panel: recent \emph{Suspicious} and \emph{Fraud} items.
  \item Chart: fraud probability over time with threshold overlay (canvas).
  \item Feed: append-only table with timestamp, request id, amount, probability, risk, label, threshold, latency, source.
\end{itemize}

\subsection*{6.2 Transaction monitoring and abnormal detection}
\begin{itemize}
  \item Every generated transaction triggers a backend \code{/predict} call; the UI uses the returned \code{fraud\_probability} as the primary signal.
  \item Risk classification rule is implemented in \code{frontend/app.js} (Normal/Suspicious/Fraud).
...
\begin{itemize}
  \item Non-fraud (0): 284{,}315 (\(\approx 99.8273\%\))
  \item Fraud (1): 492 (\(\approx 0.1727\%\))
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Demo streaming (frontend):} \code{frontend/real-samples.json} contains pre-extracted feature vectors (and optional \code{class\_label}) from the same dataset path for stable UI streaming.
...
The authoritative deployed threshold and selected model are taken from \code{artifacts/models/model\_info.json} (selection timestamp \code{2026-04-14T07:32:56.702794+00:00}). Some older/internal narrative documents in the repo may reference different model paths or thresholds; for demo correctness, follow the artifact metadata used by the API loader.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item \textbf{Frontend demo (60–90s):} start stream; narrate KPIs, feed, alerts, and chart; explain Normal/Suspicious/Fraud mapping.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \begin{itemize}
    \item Point to \code{fraud\_probability} values updating in the feed.
    \item Show Alert Panel entries for \emph{Suspicious} / \emph{Fraud}.
  \end{itemize}

  \item \textbf{Show chart}:
  \begin{itemize}
    \item Point out the rolling probability chart and the threshold line (``last\_thr'').
  \end{itemize}
